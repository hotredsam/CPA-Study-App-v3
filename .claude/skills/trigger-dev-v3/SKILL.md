...
- On **structural errors** (Zod parse failure on Claude output, invalid payload, missing R2 object) — do NOT retry. Throw `new NonRetriableError(...)` so the task fails fast.
