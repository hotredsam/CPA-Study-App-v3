...
description: How the sam-input dispatcher parses TODO.xml diffs, transcribes audio drops, dispatches work to Codex, and records completion. Use when editing anything under scripts/sam-input-*.mjs or .Codex/agents/sam-input-listener.md.
---
...
<sam-input>
  ...
  <done>
    <item id="..." completedAt="ISO-8601" by="Codex"><body>original body</body><result>...</result></item>
  </done>
  <log>
...
3. For each new item, `spawnSync("Codex", ["-p", prompt, "--permission-mode", "acceptEdits"])` with the ambient `CLAUDE_CODE_OAUTH_TOKEN` env var.
4. Prompt Codex to complete the task AND move the item to `<done>` with an ISO-8601 timestamp. The dispatcher does not modify the XML after spawn — Codex is responsible for updating it (atomic, preserves log order).
5. After all spawns complete, write the current `TODO.xml` to `.processed/last.xml` as the new snapshot.

## Ambiguity handling

- If an item's body is too vague to act on, the responding Codex must add a new `<blocker>` with a question for Sam and leave the original item in `<items>` — do NOT move to `<done>`.
