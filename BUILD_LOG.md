...
- `2026-05-06` - Production deploy branch pass: added signed-cookie login using `AUTH_SECRET`, `APP_LOGIN_EMAIL`, and `APP_LOGIN_PASSWORD_HASH` without adding third-party auth dependencies; added `pnpm auth:hash`; added mobile bottom safe-area navigation, touch-sized tab/action controls, mobile topic grid behavior, `/record` upload fallback for native iPhone screen recordings, and `/anki` Audio concept review using browser speech synthesis and existing Anki review ratings. Documented Vercel env vars, mobile Screen Capture API limits, runtime mobile probe, and NAS archive constraints.
- `2026-05-06` - Production hardening polish: moved middleware into `src/` so the `src/app` deployment actually runs the login gate, verified `AUTH_REQUIRED=true` redirect/API/login/logout behavior, made malformed password hashes fail closed, set the login route to the Node runtime, and widened production CSP just enough for Google fonts plus R2 presigned uploads/media.
